...
from fmeval.transforms.util import GeneratePrompt, GetModelResponse, PromptComposer, GenerateDeltaScores
...
get_perturbed_summ_acc_scores = [
    SummarizationAccuracy(
        ...
        meteor_score_output_key=f"{perturbed_model_response_key}.{METEOR_SCORE}",
        rouge_score_output_key=f"{perturbed_model_response_key}.{ROUGE_SCORE}",
        bertscore_output_key=f"{perturbed_model_response_key}.{BERT_SCORE}"
    )
...
perturbed_score_keys = {
    original_score_key: [
        ...
        for score_name in summ_acc.transforms
    ]
